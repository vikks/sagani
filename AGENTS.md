...
16. **Modular Documentation & GitHub Actions Security**: Keep `README.md` lean and focused on quick start installation; store detailed guides in `docs/` (`configuration.md`, `architecture.md`, `keymaps-commands.md`, `examples.md`, `contributing.md`). All GitHub Action workflows MUST define an explicit least-privilege `permissions` block (e.g. `permissions: { contents: write }`) and trigger strictly on maintainer-restricted events (`push` to `main`, `workflow_dispatch`). Unreleased codebase rule: Since sagani.nvim is pre-release, omit legacy backward-compatibility shims, deprecation warning infrastructure, and redundant top-level facade files (`diff.lua`, `picker.lua`, `backend.lua`). Note: GitHub `.wiki.git` repos return `404 Not Found` until initialized once via the browser UI; use `git push origin HEAD` and graceful fallback checks in sync workflows.
17. **Modular Test Directory Hierarchy & Spec Naming Standards**: All test files MUST be organized into domain-aligned subdirectories matching the codebase architecture (`tests/unit/` for `backend/`, `diff/`, `protocol/`, `selection/`, `state/`; `tests/integration/` for high-level workflows & plugin specs; `tests/stress/` for adversarial edge cases & benchmarks). All test files MUST follow the canonical `<domain>_spec.lua` naming convention. Nested test files MUST resolve project root via `local project_root = _G.SAGANI_PROJECT_ROOT or vim.fn.fnamemodify(debug.getinfo(1, "S").source:sub(2), ":p:h:h:h")`, and `tests/run_tests.lua` MUST recursively glob `**/*_spec.lua`.
18. **Multi-Flavor Testing Protocol & Dev Mode Resolution**: Testing `sagani.nvim` locally across multiple Neovim flavors (Vanilla, Kickstart, LazyVim, AstroNvim, NvChad) MUST use nested `NVIM_APPNAME` path subdirectories under `~/.config/nvim-flavors/` (e.g. `NVIM_APPNAME=nvim-flavors/vanilla`, `NVIM_APPNAME=nvim-flavors/kickstart`). Plugin specification files (`plugins/sagani.lua`) MUST dynamically inspect `vim.env.NVIM_PLUGIN_DEV_MODE` or local directory existence to resolve `dir` vs production git URLs.
19. **Deterministic Resolution Pipeline & Declarative Capabilities Architecture**: All task dispatch operations MUST use `sagani.resolver.build_plan(task_type, prompt, opts, session_state)` to compile raw requests into an immutable, frozen `ExecutionPlan` table with zero runtime ambiguity. System-level IPC protocols (`lua/sagani/protocol/`) MUST remain strictly decoupled from agent harness definitions (`lua/sagani/agents/`). Backend adapters MUST expose declarative geometry capabilities (`M.capabilities = { placements = { ... }, float = bool, split = bool, tab = bool, pane = { ... } }`), and agent harnesses MUST expose declarative feature capabilities (`M.capabilities = { protocols = { ... }, streaming = bool, code_completion = bool, multi_turn = bool, files = bool, media = { ... } }`).
20. **Input vs Output Formatter Package Architecture (`lua/sagani/format/`)**: Prompt and response formatting MUST be strictly separated into dedicated Single Responsibility Principle (SRP) sub-packages: `lua/sagani/format/input/` (`context.lua`, `diff.lua`, `instructions.lua`, `modes.lua`) for payloads sent TO agents, and `lua/sagani/format/output/` (`popup.lua`, `markdown.lua`, `error.lua`) for UI elements and responses received FROM agents. `lua/sagani/format/init.lua` acts as a master re-export facade guaranteeing 100% backwards compatibility for `require("sagani.format")`.
21. **Neovim Operational Modes vs Prompt Personas Architecture**: Neovim operational modes (`lua/sagani/modes/` — `edit_review.lua`, `learn.lua`) control HOW Neovim renders and behaves (interactive buffer edit diff review, inline virtual text, side-by-side splits, hunk accept/reject, educational explanation windows). Prompt personas (`lua/sagani/personas/` — `tutor.lua`, `refactor.lua`, `audit.lua`, `custom.lua`) control WHAT prompt text prefixes and LLM system instruction profiles are sent to agents. `:SaganiMode` (`<leader>am`) switches operational modes, while `:SaganiPersona` (`<leader>aP`) switches prompt persona profiles.
