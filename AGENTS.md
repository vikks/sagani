...
14. **ACP HTTP Daemon Process Lifecycle & Protocol Constraints**: When spawning background ACP daemons (`opencode acp`), pipe a persistent stream to `stdin` (`nohup sh -c 'tail -f /dev/null | opencode acp --port <port>' > /dev/null 2>&1 &`) so the daemon does not terminate on `stdin` EOF. Always use OS process table checks (`lsof -ti :<port>`) as the primary fast-path check before issuing HTTP health probes. Pass structured model objects (`{ "model": { "providerID": "...", "modelID": "..." } }`) in HTTP message payloads, and filter out internal `reasoning`/`thought` parts from JSON response streams.
