...
21. **Modular Operating Modes Strategy Package Architecture (`lua/sagani/modes/`)**: Operating modes MUST be implemented as independent strategy pattern modules under `lua/sagani/modes/` (`learn.lua`, `review.lua`, `refactor.lua`, `custom.lua`). Each mode strategy module MUST expose standard strategy properties (`id`, `name`, `icon`, `prompt_prefix`, `default_instructions`, `ui_placement`, `decorate_prompt`). `lua/sagani/modes/init.lua` acts as a master strategy facade re-exporting `get_strategy(mode_name)` and `list_modes()`.
